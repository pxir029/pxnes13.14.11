...
connections: dict = {}
CATEGORIES: dict = {}
NODES: dict = {}  # multi-location nodes
...
async def load_state():
    ...
    global AUTH, SESSIONS, SESSION_META, NODES
...
async def create_auto_link(
    ...
    config_count = safe_int(body.get("config_count", 1), minimum=1, maximum=40)
    use_nodes = bool(body.get("use_nodes") or body.get("multi_location") or body.get("multilocation"))
    uid, link = await make_link(
        ...
        note=f"Auto generated by PXPanel | profile={profile} | multi={use_nodes}",
        protocol=protocol, fingerprint=cfg["fp"],
        alpn=DEFAULT_ALPN_BY_PROTOCOL.get(protocol, ""), port=8080, fragment=cfg["fragment"],
        config_count=config_count,
    )
    link["security_profile"] = profile
    link["use_nodes"] = use_nodes
    async with LINKS_LOCK:
        LINKS[uid] = link
    await save_state()
    result = {**get_link_info(link, uid, host), "ok": True, "profile": profile, "use_nodes": use_nodes, "port": 8080}
...
async def create_auto_ai_link(
    ...
    use_nodes = bool(body.get("use_nodes") if "use_nodes" in body or "multi_location" in body else True)
    if "multi_location" in body:
        use_nodes = bool(body.get("multi_location"))
    if "use_nodes" in body:
        use_nodes = bool(body.get("use_nodes"))
    uid, link = await make_link(
        ...
        speed_limit_bytes=0,
        connection_limit=0,
        note="AI pack | Gemini / ChatGPT / Claude | WS + XHTTP",
        protocol="xhttp-stream-one",
        fingerprint="chrome",
        alpn="h2",
        port=8080,
        fragment="off",
...
@app.get("/api/nodes")
async def api_list_nodes(request: Request, _=Depends(require_auth)):
    async with LINKS_LOCK:
        items = []
...
@app.post("/api/nodes")
async def api_create_node(request: Request, _=Depends(require_auth)):
    try:
        body = await request.json()
    except Exception:
        raise HTTPException(400, detail="JSON نامعتبر")
    name = str(body.get("name") or "").strip() or "Node"
    location = str(body.get("location") or body.get("country") or "").strip() or "Unknown"
    host = str(body.get("host") or body.get("domain") or "").strip().lower()
    host = host.replace("https://", "").replace("http://", "").split("/")[0].split(":")[0]
    if not host:
        raise HTTPException(400, detail="آدرس/دامنه Node الزامی است")
    port = safe_int(body.get("port", 443), minimum=1, maximum=65535)
    ...
    async with LINKS_LOCK:
        NODES[nid] = node
    await save_state()
    log_activity("node", f"Node «{name}» ({location}) اضافه شد: {host}", "ok")
    return {"ok": True, "id": nid, **node}


@app.put("/api/nodes/{nid}")
async def api_update_node(nid: str, request: Request, _=Depends(require_auth)):
    if nid not in NODES:
        raise HTTPException(404, detail="Node پیدا نشد")
    try:
        body = await request.json()
    except Exception:
        raise HTTPException(400, detail="JSON نامعتبر")
    async with LINKS_LOCK:
        n = NODES[nid]
        if "name" in body:
            n["name"] = str(body.get("name") or n["name"]).strip()
        if "location" in body or "country" in body:
            n["location"] = str(body.get("location") or body.get("country") or n["location"]).strip()
        if "host" in body or "domain" in body:
            host = str(body.get("host") or body.get("domain") or n["host"]).strip().lower()
            host = host.replace("https://", "").replace("http://", "").split("/")[0].split(":")[0]
            n["host"] = host
        if "port" in body:
            n["port"] = safe_int(body.get("port"), minimum=1, maximum=65535)
        if "enabled" in body:
            n["enabled"] = bool(body.get("enabled"))
    await save_state()
    return {"ok": True, "id": nid, **NODES[nid]}


@app.delete("/api/nodes/{nid}")
async def api_delete_node(nid: str, _=Depends(require_auth)):
    async with LINKS_LOCK:
        n = NODES.pop(nid, None)
    if not n:
        raise HTTPException(404, detail="Node پیدا نشد")
    await save_state()
    log_activity("node", f"Node «{n.get('name')}» حذف شد", "ok")
    return {"ok": True}
...
async def subscription_single(
    ...
    clean_ips = link.get("clean_ips") or []
    enabled_nodes = [
        {"host": n.get("host"), "location": n.get("location") or n.get("name") or "Node", "port": int(n.get("port") or 443), "name": n.get("name") or "Node"}
        for n in NODES.values()
        if n.get("enabled") and n.get("host")
    ]
    ...
    if clean_ips:
        ...
        # Multi-location hosts: main panel + enabled nodes
        main_port = int(link.get("port") or DEFAULT_PORT or 8080)
        loc_hosts = [{"host": host, "location": "Main", "port": main_port}]
        if link.get("use_nodes"):
            for en in enabled_nodes:
                if en.get("host") and en["host"] != host:
                    loc_hosts.append({
                        "host": en["host"],
                        "location": en.get("location") or en.get("name") or "Node",
                        "port": int(en.get("port") or 443),
                        "name": en.get("name") or "Node",
                    })
        for i in range(cfg_count):
            proto = _proto_for_index(i)
            loc = loc_hosts[i % len(loc_hosts)]
            h = loc["host"]
            pport = int(loc.get("port") or link.get("port") or DEFAULT_PORT or 443)
            if is_ai_pack:
                name = _ai_remark(i, proto)
                name = f"{name} | {loc.get('location') or 'Node'}"
            else:
                base = random_config_name(used_names)
                name = f"{base} | {loc.get('location') or 'Node'}"
            used_names.add(name)
            lines.append(generate_vless_link(
                uuid, h, remark=name, protocol=proto,
                fingerprint=_fp_for_proto(proto), alpn=_alpn_for_proto(proto),
                port=pport,
            ))
